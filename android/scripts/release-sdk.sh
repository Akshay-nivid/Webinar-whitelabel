...
echo "Releasing Confgo SDK ${SDK_VERSION}"
...
# Now build and publish the Confgo SDK and its dependencies
echo "Building and publishing the Confgo SDK"
pushd ${THIS_DIR}/../
./gradlew clean
...
if [[ $MVN_HTTP == 0 ]]; then
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Confgo SDK + dependencies: ${SDK_VERSION}"
    popd
fi
